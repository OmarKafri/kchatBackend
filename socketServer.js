...
io.on("connection", (socket) => {
  ...
  socket.on("send_message", async (msg) => {
    ...
    console.log("Message content:", msg.content_text);

    if (msg.content_text && msg.content_text.startsWith("@kchat")) {
      const userQuestion = msg.content_text.replace("@kchat", "").trim();
      
      try {
        const questionRes = await fetch(`${NEXTJS_API_URL}/api/messages`, {
          method: "POST",
          headers: {
        ...
        if (questionRes.ok) {
          ...
          io.to(msg.conversation_id).emit("receive_message", savedQuestion);
          io.to(msg.conversation_id).emit("read_message", savedQuestion);
        }
      } catch (err) {
        console.error("Error saving user question:", err.message);
      }
      ...
      try {
        ...
        const botMessageRes = await fetch(`${NEXTJS_API_URL}/api/messages`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (botMessageRes.ok) {
          const savedBotMessage = await botMessageRes.json();
          io.to(msg.conversation_id).emit("receive_bot_message", savedBotMessage);
          io.to(msg.conversation_id).emit("read_message", savedBotMessage);
          console.log(`Bot message saved & sent to room: ${msg.conversation_id}`);
        } else {
  ...
  socket.on("call-user", (obj) => {
    io.to(obj.receiverId).emit("send_call_request", {
      senderId: obj.senderId,
      receiverId: obj.receiverId,
    });
  });
  socket.on("reject_call", (obj) => {
    io.to(obj.senderId).emit("call_rejected", {
      senderId: obj.senderId,
      receiverId: obj.receiverId,
    });
  });
  socket.on("call_ended", async (obj) => {
    try {
      const conversationRes = await fetch(
        `${NEXTJS_API_URL}/api/conversation`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const senderRes = await fetch(
        `${NEXTJS_API_URL}/api/user?userID=${obj.senderId}`
      );
      ...
      io.to(obj.senderId).emit("no-response", {
        senderId: obj.senderId,
        receiverId: obj.receiverId,
      });

      console.log(
        `Missed call message saved for conversation: ${conversationId}`
      );
    } catch (err) {
  ...
  socket.on("accept_call", async (obj) => {
    try {
      ...
      const receiverRes = await fetch(
        `${NEXTJS_API_URL}/api/user?userID=${obj.receiverId}`
      );
